#!/usr/bin/env python
"""
Tune sample data for the Reading Overview dashboard so KPIs look realistic.

This script is SAFE to run multiple times and is intended ONLY for sample data.
It adjusts Reading assessment scores so that, for a chosen school year:
...
- Loads `v_support_status` for Reading via `core.database.get_v_support_status`.
- For a subset of enrollments with valid thresholds it:
  ...
  - Optionally nudges some scores between thresholds to approximate the desired
    **Monitor** count.
- Applies updates by modifying the *latest* Reading assessment per enrollment
...
Usage
-----
    python scripts/tune_reading_overview_sample_data.py \
        --school-year 2024-25 \
        --target-monitor 15 \
        --target-needs 10 \
        --apply
...
def _print_kpis(label: str, school_year: str) -> None:
    k = get_teacher_kpis(school_year=school_year, subject="Reading")
    print(f"\n[{label}] Reading KPIs for {school_year}:")
    print(
...
def _load_support_status(school_year: str) -> pd.DataFrame:
    """Load v_support_status rows for Reading + given school_year, with thresholds."""
...
def _apply_adjustments(adjustments: list[ScoreAdjustment], dry_run: bool = True) -> None:
    """Apply score adjustments to the latest Reading assessment per enrollment."""
    if not adjustments:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Tune Reading Overview KPIs for sample data.")
    ...
    print(f"Tuning Reading Overview for school_year={school_year!r}")

    _print_kpis("BEFORE", school_year)

    df = _load_support_status(school_year)
    if df.empty:
        print("\nNo v_support_status rows found for Reading; nothing to tune.")
        return
