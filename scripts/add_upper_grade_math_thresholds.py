#!/usr/bin/env python3
"""
Add benchmark_thresholds rows for upper-grade Math (5th–8th).

This ensures v_support_status can compute Tier / Needs Support for 5th–8th
Math enrollments, so the dashboard and tuning script can include them.

Logic:
- Find distinct (subject_area, assessment_type, grade_level, assessment_period, school_year)
  combinations for Math assessments where the enrollment's grade is 5th–8th.
...
Usage:
    python scripts/add_upper_grade_math_thresholds.py
"""
...
def main() -> None:
    ...
    # Distinct Math assessment combos for 5th–8th with enrollments wired up.
    cur.execute(
        """
    ...
    rows = cur.fetchall()
    if not rows:
        print("No Math assessments found for upper grades with enrollments; nothing to do.")
        conn.close()
        return
    ...
    if not to_insert:
        print("All upper-grade Math benchmark_thresholds already present; nothing to insert.")
        conn.close()
        return
    ...
    print(f"Inserted {len(to_insert)} new upper-grade Math benchmark_threshold rows.")
